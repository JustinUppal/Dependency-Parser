...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        def get_word_idx(word_id):
            ...
            word_str = words[word_id - 1]
            if word_str is None:
                return self.word_vocab['<UNK>']
            word_pos = pos[word_id - 1]
            ...
            return self.word_vocab.get(word_str_lower, self.word_vocab['<UNK>'])
        
        pad_stack = ([None] * 3 + state.stack)[-3:]
        pad_buffer = ([None] * 3 + state.buffer)[-3:]
        pad_stack.reverse()
        pad_buffer.reverse()
        stack_idx = [get_word_idx(id) for id in pad_stack]
        buffer_idx = [get_word_idx(id) for id in pad_buffer]
        feature_idx = stack_idx + buffer_idx
        return np.array(feature_idx)

    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
        output_idx = self.output_labels.get(output_pair)
        res = np.zeros(91)
